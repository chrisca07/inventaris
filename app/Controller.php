...
class Controller
{
    ...
    public function Lokasi()
    {
        ...
        //$this->fields('barang');
        $fields1 = '[
                {"name":"nama_lokasi","label":"Nama Lokasi","type":"text","max":"25","pnj":12,"val":null,"red":"","input":true,"up":true,"tb":true,"var":"input[]","var2":"tb[]"}
                ]';
        $data['lokasi.form'] = json_decode($fields1, true);
        $data['created_at'] = date('Y-m-d');

        if (isset($this->Request->tgl)) {
            $data['created_at'] = $this->Request->tgl;
        }
        $data['lokasi'] = collect($this->Crud->mysqli2->table('lokasi')->select()->get());
    ...
    public function Mutasi()
    {
        ...
        $fields1 = '[
               ...
               {"name":"merk","label":"Merk","type":"text","max":"15","pnj":12,"val":null,"red":"","input":false,"up":false,"tb":true,"var":"input[]","var2":"tb[]"},
                ...
                {"name":"kategori","label":"Kategori","type":"text","max":"25","pnj":12,"val":null,"red":"","input":false,"up":false,"tb":true,"var":"input[]","var2":"tb[]"},
               {"name":"qty","label":"Qty","type":"number","max":null,"pnj":12,"val":null,"red":"","input":true,"up":true,"tb":true,"var":"input[]","var2":"tb[]"},
               {"name":"idlokasi","label":"lokasi","type":"number","max":null,"pnj":12,"val":null,"red":"","input":true,"up":true,"tb":false,"var":"input[]","var2":"tb[]"},
               {"name":"nama_lokasi","label":"Nama Lokasi","type":"text","max":"15","pnj":12,"val":null,"red":"","input":false,"up":false,"tb":true,"var":"input[]","var2":"tb[]"},
               {"name":"jenismutasi","label":"Jenis Mutasi","type":"text","max":"15","pnj":12,"val":null,"red":"","input":true,"up":true,"tb":true,"var":"input[]","var2":"tb[]"},
               {"name":"ket","label":"Keterangan","type":"text","max":"100","pnj":12,"val":null,"red":"","input":true,"up":true,"tb":true,"var":"input[]","var2":"tb[]"}
                ]';
        $data['mutasi.form'] = json_decode($fields1, true);
        $data[$data['table'] . '.form'] = json_decode($fields1, true);
        //dd($data['mutasi.form']);
        $data['tgl'] = date('Y-m-d');
        ...
        $data['mutasi'] = collect($this->Crud->mysqli2->table('mutasi')->select()
        ->join('barang', 'barang.idbarang', '=', 'mutasi.idbarang') 
        ->join('lokasi','lokasi.idlokasi','=','mutasi.idlokasi')
        ->where('mutasi.tgl', $data['tgl'])->get());
        $data['barang'] = collect($this->Crud->mysqli2->table('barang')->select()->get())->groupBy('kategori');
        $data['lokasi'] = collect($this->Crud->mysqli2->table('lokasi')->select()->get());
    ...
    public function Inventaris()
    {
        ...
        if (isset($data['Request']->tgl)) {
            //dd($data['Request']->tgl[1]);
            //dd($data['Request']->tgl);
            //dd($data['Request']->jenis);
            $data['transaksi'] = $this->Crud->mysqli2->table('transaksi')->select('transaksi.qty,barang.nama_barang,transaksi.jenis,barang.idbarang')->join('barang', 'barang.idbarang', '=', 'transaksi.idbarang')->where(new Ex('year(tgl)'), $data['Request']->tgl[1]);
            //dd($data['transaksi']);
    ...
    public function Stok()
    {
        ...
        $data['barang.form'] = json_decode($fields1, true);
        $data['transaksi'] = collect($this->Crud->mysqli2->table('transaksi')->select()->get());
        $data['mutasi'] = collect($this->Crud->mysqli2->table('mutasi')->select()->get());
        //dd($data['transaksi']);
        $data['barang'] = collect($this->Crud->mysqli2->table('barang')->select()->get())->map(function ($item, $key) use ($data) {
            $ts = $data['transaksi']->where('idbarang', $item->idbarang);
            //dd($ts);
            $mutasi = $data['mutasi']->where('idbarang', $item->idbarang);
             // Hitung total qty masuk dari mutasi
            $qty_mutasi_masuk = $mutasi->where('jenismutasi', 'Masuk')->sum('qty');
            
            // Hitung total qty keluar dari mutasi
            $qty_mutasi_keluar = $mutasi->where('jenismutasi', 'Keluar')->sum('qty');
            $item->stok = $ts->where('jenis', 'Masuk')->sum('qty') + $qty_mutasi_masuk - $ts->where('jenis', 'Keluar')->sum('qty') - $qty_mutasi_keluar;
            return $item;
        });
