        ...
        <div class="card rounded shadow" style="zoom:85%">
            ...
            <div class=" card-body ">
                <form onsubmit="return validateForm()" action="Action.php" method="post" enctype="multipart/form-data">
                    <div class="row">
                        <?php foreach ($data['lokasi.form'] as $isi): ?>
                        ...
                        <input type="hidden" name="input[]" value="<?php echo $data['created_at']; ?>">
                        <input type="hidden" name="tb[]" value="created_at">
                            <input type="hidden" name="table" value="lokasi">
        ...
        <div class="card rounded shadow" style="zoom:85%">
            ...
            <table width="100%" class="text-wrap mb-0 tb table table-borderless table-striped table-hover ">
                <thead class="">
                    <tr>
                        <th class="w-1">#</th>
                        <?php foreach ($data['lokasi.form'] as $e): ?>
                        <?php if ($e['tb']): ?>
                        <th class="">
                            <?php echo $e['label']; ?>
                        </th>
                ...
                <tbody>
                    <?php foreach ($data['lokasi'] as $v => $e): ?>
                    <tr>
                        <td>
                            <?php echo $v + 1; ?>
                        </td>
                        <?php foreach ($data['lokasi.form'] as $e1): ?>
                        <?php if ($e1['tb']): ?>
                        <td class="text-wrap">
                            <?php $b = $e1['name'];?>
                            <?php echo $e->$b; ?>
                        ...
                        <?php endforeach;?>
                        <td class="text-right ">
                           <span style="display: none" id="data-<?php echo $e->idlokasi; ?>">
                                <?php echo json_encode($e); ?></span>
                            <a class="mr-1 text-info" onclick="app.kd=JSON.parse($('#data-<?php echo $e->idlokasi; ?>').html())" data-toggle="modal" data-target="#modal-edit" href="javascript:void(0)">
                                <i class="fa fa-edit"></i>
                            </a>
                            <a class=" text-danger" onclick="return confirm('Apakah anda yakin ingin hapus data ini?');" href="Action.php?aksi=delete&table=lokasi&primary=idlokasi&key=<?php echo $e->idlokasi; ?>">
                ...
                <div class="modal-body  " style="background: rgb(240, 241, 245)">
                    <div style="zoom:85%" class="card card-body ">
                        <div class="row">
                            ...
                            <div class="modal-footer col-12  py-1">
                                <input type="hidden" name="table" value="lokasi">
                                <input type="hidden" name="primary" value="idlokasi">
                                <input type="hidden" name="key" :value="kd.idlokasi">
